...
amp_sm_tmp = fftfilt(hanning(sample_distance), amp_sq .* amp_sq(:, [1 1:end-1]));
gd = abs(outputDs.group_delay_map);
gd_sm_tmp = fftfilt(hanning(sample_distance), amp_sq .* amp_sq(:, [1 1:end-1]) .* gd .* gd(:, [1 1:end-1]));
...
amp_sm_tmp_if = fftfilt(hanning(sample_distance), amp_sq .* amp_sq([1 1:end-1], :));
...
if_sm_tmp = fftfilt(hanning(sample_distance), amp_sq .* amp_sq(:, [1 1:end-1]) .* dif_map .* dif_map(:, [1 1:end-1]));
...
if_smoothed = fftfilt(hanning(sample_distance), outputDs.inst_freq_map .* outputDs.amp_squared_map);
amp_smoothed = fftfilt(hanning(sample_distance), outputDs.amp_squared_map);
